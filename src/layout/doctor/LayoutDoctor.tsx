...
const LayoutDoctor = () => {
    ...
    const menuItems = [
        ...
        },
        {
            ...
            items: [
                { key: 'doctor3', icon: <FaUserDoctor />, label: 'Bác sĩ', path: `cau-hinh-lich/bac-si/${id}` },

                ...filteredServices?.map((service: any) => ({
                    key: `config-${service.id.toString()}`,
                    label: service.doctorName,
                    path: `cau-hinh-lich/dich-vu/${service.id}`,
                })) || [],
            ],
        },
        {
            ...
            label: 'Cấu hình các ngày trong tuần',
            items: [
                { key: 'doctor4', icon: <FaUserDoctor />, label: 'Bác sĩ', path: `cau-hinh-ngay-trong-tuan/bac-si/${id}` },

                ...filteredServices?.map((service: any) => ({
                    key: `configweek-${service.id.toString()}`,
                    label: service.doctorName,
                    path: `cau-hinh-ngay-trong-tuan/dich-vu/${service.id}`,
                })) || [],
            ],
        },
    ];
